...
if_unsigned!{ $_ty, pub type const CAST_SIGNED<const N: $ty>: $ty_signed = const { <$ty>::cast_signed(N) }; }
...
if_unsigned!{ $_ty, pub type const IS_MULTIPLE_OF<const L: $ty, const R: $ty>: bool = const { <$ty>::is_multiple_of(L, R) }; }
if_unsigned!{ $_ty, pub type const IS_POWER_OF_TWO<const N: $ty>: bool = const { <$ty>::is_power_of_two(N) }; }
...
if_unsigned!{ $_ty, pub type const NEXT_MULTIPLE_OF<const L: $ty, const R: $ty>: $ty = const { <$ty>::next_multiple_of(L, R) }; }

if_unsigned!{ $_ty, pub type const NEXT_POWER_OF_TWO<const N: $ty>: $ty = const { <$ty>::next_power_of_two(N) }; }

                    pub type const OVERFLOWING_ADD<const L: $ty, const R: $ty>: ($ty, bool) = const { <$ty>::overflowing_add(L, R) };
if_unsigned!{ $_ty, pub type const OVERFLOWING_ADD_SIGNED<const L: $ty, const R: $ty_signed>: ($ty, bool) = const { <$ty>::overflowing_add_signed(L, R) }; }
...
if_unsigned!{ $_ty, pub type const OVERFLOWING_SUB_SIGNED<const L: $ty, const R: $ty_signed>: ($ty, bool) = const { <$ty>::overflowing_sub_signed(L, R) }; }
...
if_unsigned!{ $_ty, pub type const SATURATING_ADD_SIGNED<const L: $ty, const R: $ty_signed>: $ty = const { <$ty>::saturating_add_signed(L, R) }; }
...
if_unsigned!{$_ty,  pub type const SATURATING_SUB_SIGNED<const L: $ty, const R: $ty_signed>: $ty = const { <$ty>::saturating_sub_signed(L, R) }; }
